...
<html>
  ...
  <body>
    ...
    <div class="modal hide" id="modal_add">
      <form action="add5.php" id="modal_addform" class="form-horizontal">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">×</button>
          <h3>Add something!</h3>
        </div>
    ...
    <script>
      ...
      $("#modal_addform").submit(function(event) {
        ...
        var $form = $( this ),
            owner_name1 = $form.find( '#add_owner_name' ).val(),
            owner_email1 = $form.find( '#add_owner_email' ).val(),
            title1 = $form.find( '#add_title' ).val(),
            type1 = $form.find( '#add_type' ).val(),
            address1 = $form.find( '#add_address' ).val(),
            uri1 = $form.find( '#add_uri' ).val(),
            description1 = $form.find( '#add_description' ).val(),
            employeenum1 = $form.find( '#employeenum' ).val(),
        ...
        $.ajax({
          type: "POST",
          url : "add5.php",
          data: {owner_name: owner_name1, owner_email: owner_email1, title: title1, type: type1, address: address1, uri: uri1, description: description1, employeenum: employeenum1}
        }).done (function (data) {
          alert("here");
